...
* Postgresql
    ...
    - Завжди використовуватися набір обов'язкових додаткових полів:
        ...
        - id (тип: uuid, дефолтне значення: "gen_random_uuid()")
            - Потребує встановленого пакету postgresql-contrib
            - Потребує реєстрації у базі даних  "CREATE EXTENSION IF NOT EXISTS pgcrypto;"

* Python

## Структура файлова та у базі даних

### Файлова

minjust         \ Назва розпорядника
    opendata    \ відкриті дані за постановою 835
        01      \ Порядковий номер у постанові (за редакцією 2019 року)
    public      \ Публічні дані

### У базі данних

    - opendata  \ назва бази у postgresql
    - minjustOD01name
        minjust - назва розпорядника
        OD/PD   - відкриті дані або публічні
        01      - порядковий номер відкритих даних згідно постанови 835 (за редакцією 2019 року)
        name    - назва окремої таблиці в кластері реєстру
